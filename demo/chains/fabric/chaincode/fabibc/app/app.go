...
import (
	...
	crosssimapp "github.com/datachainlab/cross/simapp"
	...
	crossante "github.com/datachainlab/cross/x/core/auth/ante"
	...
	erc20mgrante "github.com/datachainlab/fabric-besu-cross-demo/contracts/erc20/erc20mgr/ante"
	erc20mgrkeeper "github.com/datachainlab/fabric-besu-cross-demo/contracts/erc20/erc20mgr/keeper"
...
func NewIBCApp(appName string, logger log.Logger, db dbm.DB, traceStore io.Writer, encodingConfig simappparams.EncodingConfig, seqMgr commitment.SequenceManager, blockProvider app.BlockProvider, anteHandlerProvider app.AnteHandlerProvider) (*IBCApp, error) {
	...
	keys := sdk.NewKVStoreKeys(
		...
		crosstypes.StoreKey, erc20types.StoreKey, erc20mgrtypes.StoreKey,
	)
	...
	schemaPrefix := []byte(erc20mgrtypes.ModuleName + "/")
	schema.Set(schemaPrefix, cdttypes.CDT_TYPE_INT64)
	xstore := cdtkeeper.NewStore(appCodec, keys[crosstypes.StoreKey], schema)
	int64Store := xstore.GetInt64Store(schemaPrefix)
	app.ERC20Keeper = erc20keeper.NewKeeper(int64Store)
	...
	anteHandler, err := NewAnteHandler(crosssimapp.HandlerOptions{})
	if err != nil {
		panic(err)
	}
...
func NewAnteHandler(options crosssimapp.HandlerOptions) (sdk.AnteHandler, error) {
	sigHandler := func(ctx sdk.Context, tx sdk.Tx, simulate bool) (newCtx sdk.Context, err error) {
		return ctx, nil
	}
	...
	anteDecorators := []sdk.AnteDecorator{
		...
		erc20mgrante.NewFabricVerificationDecorator(),
